...
// SetAdvertisingRule - This function is a service function that is used to set an advertisement rule for the application. It checks for
...
func (s *Service) SetAdvertisingRule(ctx context.Context, req *pbads.SetRequestAdvertisementsRule) (*pbads.ResponseAdvertising, error) {

	// The purpose of the above code is to declare two variables in the same statement. The first variable, response, is of
	// type pbads.ResponseAdvertising. The second variable, migrate, is of type query.Migrate and has a Context field set to s.Context.
	var (
	...
	// Advertising object is equal to the pbads.Type_TEXT type, then the code within the block will be executed.
	if req.Advertising.GetType() == pbads.Type_TEXT {
	...
	if req.GetId() > 0 {
		...
		if _, err := s.Context.Db.Exec("update advertising set title = $1, text = $2, link = $3, type = $4 where id = $5;",
		...
		if err := s.Context.Db.QueryRow("insert into advertising (title, text, link, type) values ($1, $2, $3, $4) returning id;",
			req.Advertising.GetTitle(),
			req.Advertising.GetText(),
			req.Advertising.GetLink(),
			req.Advertising.GetType(),
...
// DeleteAdvertisingRule - This function is used to delete an advertising record from a database. It first checks for authorization and the
...
func (s *Service) DeleteAdvertisingRule(ctx context.Context, req *pbads.DeleteRequestAdvertisingRule) (*pbads.ResponseAdvertising, error) {

	// The purpose of this code is to create a new variable 'response' of type pbads.ResponseAdvertising and a new variable
	// 'migrate' of type query.Migrate. The migrate variable is given the s.Context value, which is the context associated
