...
func (s *Service) GetAdvertisements(_ context.Context, req *pbads.GetRequestAdvertisements) (*pbads.ResponseAdvertising, error) {
	...
	if _ = s.Context.Db.QueryRow("select count(*) as count from advertising where type = $1", req.GetType()).Scan(&response.Count); response.GetCount() > 0 {
...
func (s *Service) GetAdvertising(ctx context.Context, req *pbads.GetRequestAdvertising) (*pbads.ResponseAdvertising, error) {
	...
	// points (id, title, text, link, type) associated with the given ID and assigning them to the item object. This allows
	// the item object to store the data points associated with the given ID.
	_ = s.Context.Db.QueryRow("select id, title, text, link, type from advertising where id = $1", req.GetId()).Scan(&item.Id, &item.Title, &item.Text, &item.Link, &item.Type)
