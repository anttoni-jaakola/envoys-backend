...
func (s *Service) SetAgent(ctx context.Context, req *pbstock.SetRequestAgent) (*pbstock.ResponseAgent, error) {
	...
	switch req.GetType() {
	case types.TypeAgent:
		agent.Status = types.StatusPending
		agent.Type = types.TypeAgent
	case types.TypeBroker:
		agent.Status = types.StatusAccess
		agent.Type = types.TypeBroker
	default:
...
func (s *Service) GetBrokers(_ context.Context, req *pbstock.GetRequestBrokers) (*pbstock.ResponseBroker, error) {
	...
	if len(req.GetSearch()) > 0 {
		maps = append(maps, fmt.Sprintf("where type = '%[2]v' and (name like %[1]s or id::text like %[1]s)", "'%"+req.GetSearch()+"%'", types.TypeBroker))
	} else {
		maps = append(maps, fmt.Sprintf("where type = '%[1]v'", types.TypeBroker))
	}
...
func (s *Service) GetRequests(ctx context.Context, req *pbstock.GetRequestRequests) (*pbstock.ResponseAgent, error) {
	...
	if _ = s.Context.Db.QueryRow("select count(*) as count from agents where status = $1 and type = $2 and broker_id = $3", types.StatusPending, types.TypeAgent, agent.GetId()).Scan(&response.Count); response.GetCount() > 0 {
...
func (s *Service) GetAgents(ctx context.Context, req *pbstock.GetRequestAgents) (*pbstock.ResponseAgent, error) {
	...
	if _ = s.Context.Db.QueryRow("select count(*) as count from agents a left join accounts b on b.id = a.user_id where a.broker_id = $1 and a.type = $2 and a.status = $3", agent.GetId(), types.TypeAgent, types.StatusAccess).Scan(&response.Count); response.GetCount() > 0 {
		...
		// types.TypeAgent, req.GetLimit(), offset. The query will return the id, name, email, user_id, broker_id, type,
...
func (s *Service) SetBlocked(ctx context.Context, req *pbstock.SetRequestAgentBlocked) (*pbstock.ResponseBlocked, error) {
	...
	if err = s.Context.Db.QueryRow("select status as count from agents where id = $1 and broker_id = $2 and type = $3", req.GetId(), agent.GetId(), types.TypeAgent).Scan(&status); err != nil {
		return &response, err
	}
...
func (s *Service) GetAssets(ctx context.Context, req *pbstock.GetRequestAssets) (*pbstock.ResponseAsset, error) {
	...
	if req.GetFiat() {
		...
		row, err := s.Context.Db.Query(`select id, name, symbol, status from assets where "group" = $1`, types.GroupFiat)
		if err != nil {
			return &response, err
		}
		defer row.Close()
		...
		for row.Next() {
			...
			_ = s.Context.Db.QueryRow(`select value from balances where symbol = $1 and user_id = $2 and type = $3`, item.GetSymbol(), auth, types.TypeStock).Scan(&item.Balance)
		...
		row, err := s.Context.Db.Query(`select id, name, symbol, "group", zone, price, status from stocks`)
		if err != nil {
			return &response, err
		}
		defer row.Close()
		...
		for row.Next() {
			...
			if err := row.Scan(&item.Id, &item.Name, &item.Symbol, &item.Group, &item.Zone, &item.Price, &item.Status); err != nil {
				return &response, err
			}
...
func (s *Service) GetAsset(ctx context.Context, req *pbstock.GetRequestAsset) (*pbstock.ResponseAsset, error) {
	...
	// BROKER, which is a member of the types type. This is likely done to ensure that the item has the correct type for the given broker.
	if agent.GetBrokerId() == 0 {
		item.Type = types.TypeBroker
	}
	...
	// 'item.Volume'.
	_ = s.Context.Db.QueryRow(`select coalesce(sum(case when base_unit = $1 then value when quote_unit = $1 then value * price end), 0.00) as volume from orders where base_unit = $1 and status = $2 and type = $3 and user_id = $4 or quote_unit = $1 and status = $2 and type = $3 and user_id = $4`, req.GetSymbol(), types.StatusPending, types.TypeStock, auth).Scan(&item.Volume)
	...
	_ = s.Context.Db.QueryRow("select exists(select id from balances where symbol = $1 and user_id = $2 and type = $3)::bool", req.GetSymbol(), auth, types.TypeStock).Scan(&item.Exist)
	...
	if row.Next() {
		...
		// protect against SQL injection attacks. The parameters are taken from the req.GetSymbol() and types.GroupFiat
		...
		row, err = s.Context.Db.Query(`select id, name, symbol, status from assets where symbol = $1 and "group" = $2`, req.GetSymbol(), types.GroupFiat)
		if err != nil {
			return &response, err
		}
		defer row.Close()
...
func (s *Service) SetAsset(ctx context.Context, req *pbstock.SetRequestAsset) (*pbstock.ResponseAsset, error) {
	...
	// The purpose of the following code is to check if a given asset exists in the database.
	// The code is querying the database with a specific symbol, user_id and asset type and then scanning the result of the query into a boolean value which is stored in the item.Exist variable.
	if _ = s.Context.Db.QueryRow("select exists(select id from balances where symbol = $1 and user_id = $2 and type = $3)::bool", req.GetSymbol(), auth, types.TypeStock).Scan(&item.Exist); !item.Exist {
		...
		if _, err = s.Context.Db.Exec("insert into balances (user_id, symbol, type) values ($1, $2, $3);", auth, req.GetSymbol(), types.TypeStock); err != nil {
			return &response, err
		}
...
func (s *Service) CancelWithdraw(ctx context.Context, req *pbstock.CancelRequestWithdraw) (*pbstock.ResponseWithdraw, error) {
	...
	if row.Next() {
		...
		if _, err := s.Context.Db.Exec("update withdraws set status = $3 where id = $1 and user_id = $2;", req.GetId(), item.GetUserId(), types.StatusCancel); err != nil {
			return &response, err
		}
		response.Success = true
...
func (s *Service) GetPairs(_ context.Context, _ *pbstock.GetRequestPairs) (*pbstock.ResponsePair, error) {
	...
	// The purpose of this statement is to declare a variable called 'response' of the type 'pbstock.ResponsePair'. This
	// type is probably a custom type that the programmer has created and is used to store data related to stock prices.
	var (
		response pbstock.ResponsePair
	)

	// This code is querying a database for data. The purpose of this code is to select data from a table called "stocks" in
	// the database and store it in the variable "row" for further processing. The "if" statement is a check for any errors
	// that may occur while attempting the query, while the "defer" statement is used to make sure that the row is closed
	...
	// The purpose of the for row.Next() statement is to loop through a set of rows in a result set from a database query.
	// It is used to iterate through each row in the result set and perform an operation on each row.
	for row.Next() {

		// The purpose of this code is to declare a new variable called "item" of type "Pair" from the types package. This
		// declares the variable and allocates memory for it, but does not assign a value to it.
		var (
			item types.Pair
		)
...
func (s *Service) GetPrice(_ context.Context, req *pbstock.GetRequestPrice) (*pbstock.ResponsePrice, error) {
	...
	// This variable declaration is declaring a variable named 'response' with the type 'pbstock.ResponsePrice'.  This is a
	// way for the programmer to declare a variable as a specific type in order to ensure that the variable is used
	// correctly throughout the program. This type of declaration ensures that the variable is only used to store data of
	// the type 'pbstock.ResponsePrice'.
	var (
		response pbstock.ResponsePrice
	)

	// The code above is used to retrieve a stock price from a database. It does this by using the QueryRow() method from
	// the Db object, which takes in a SQL query as an argument. The query is searching for the price of a stock with a
	// specific symbol and zone, which are specified by the req.GetBaseUnit() and req.GetQuoteUnit() methods. The result of
	// the query is then stored in response.Price using the Scan() function.
...
func (s *Service) SetOrder(ctx context.Context, req *pbstock.SetRequestOrder) (*pbstock.ResponseOrder, error) {
	...
	case types.AssigningBuy:
		...
		if err := s.setBalance(order.GetQuoteUnit(), order.GetUserId(), quantity, types.BalanceMinus); err != nil {
			return &response, err
		}

		s.trade(&order, types.AssigningSell)
	...
	case types.AssigningSell:
		...
		// This code is checking the balance of a user and attempting to subtract the specified quantity from it. If the
		// operation is successful, it will continue with the program. If an error occurs, it will return an error response.
		if err := s.setBalance(order.GetBaseUnit(), order.GetUserId(), quantity, types.BalanceMinus); err != nil {
			return &response, err
		}

		s.trade(&order, types.AssigningBuy)
...
// GetOrders - The purpose of this function is to get orders from a database based on the data provided in a request object (req). It
// will build a SQL query based on the data from the req object and return a response object containing the list of
// orders from the database. The function will also perform authentication checks and return an error if an error occurs
// during the authentication process. The function will also limit the number of results returned and calculate the
// offset for a page of results. Finally, the function will return a response object containing the list of orders from the database.
func (s *Service) GetOrders(ctx context.Context, req *pbstock.GetRequestOrders) (*pbstock.ResponseOrder, error) {
	...
	case types.AssigningBuy:
		maps = append(maps, fmt.Sprintf("where assigning = '%[1]v' and type = '%[2]v'", types.AssigningBuy, types.TypeStock))
	case types.AssigningSell:
		maps = append(maps, fmt.Sprintf("where assigning = '%[1]v' and type = '%[2]v'", types.AssigningSell, types.TypeStock))
	default:
		maps = append(maps, fmt.Sprintf("where (assigning = '%[1]v' and type = '%[3]v' or assigning = '%[2]v' and type = '%[3]v')", types.AssigningBuy, types.AssigningSell, types.TypeStock))
	}
...
// CancelOrder - This code is a function in a stock service, used to cancel an order. It takes in a context and a request, and returns
// a response. The function will check for a valid authentication token, then query the database for the order, set the
// balance for the user, and publish the item to an exchange. It will then return a response indicating whether or not
...
func (s *Service) CancelOrder(ctx context.Context, req *pbstock.CancelRequestOrder) (*pbstock.ResponseOrder, error) {
	...
	// The purpose of this code is to declare a variable named response of type pbstock.ResponseOrder. This is a type used to
	...
	row, err := s.Context.Db.Query(`select id, value, quantity, price, assigning, base_unit, quote_unit, user_id, create_at from orders where status = $1 and type = $2 and id = $3 and user_id = $4 order by id`, types.StatusPending, types.TypeStock, req.GetId(), auth)
	if err != nil {
		return &response, err
	}
	defer row.Close()
	...
	if row.Next() {
		...
		if _, err := s.Context.Db.Exec("update orders set status = $1 where type = $2 and id = $3 and user_id = $4;", types.StatusCancel, types.TypeStock, item.GetId(), item.GetUserId()); err != nil {
			return &response, err
		}
		...
		case types.AssigningBuy:
			...
			if err := s.setBalance(item.GetQuoteUnit(), item.GetUserId(), decimal.New(item.GetValue()).Mul(item.GetPrice()).Float(), types.BalancePlus); err != nil {
				return &response, err
			}
		...
		case types.AssigningSell:
			...
			if err := s.setBalance(item.GetBaseUnit(), item.GetUserId(), item.GetValue(), types.BalancePlus); err != nil {
				return &response, err
			}
		...
		if err := s.Context.Publish(&item, "exchange", "order/cancel"); err != nil {
			return &response, err
		}
