...
func (e *Service) SetOrder(ctx context.Context, req *pbspot.SetRequestOrder) (*pbspot.ResponseOrder, error) {
	...
	case types.AssigningBuy:
		...
		if err := e.setBalance(order.GetQuoteUnit(), order.GetUserId(), quantity, types.BalanceMinus); err != nil {
			return &response, err
		}

		e.trade(&order, types.AssigningSell)
	...
	case types.AssigningSell:
		...
		// This code is checking the balance of a user and attempting to subtract the specified quantity from it. If the
		// operation is successful, it will continue with the program. If an error occurs, it will return an error response.
		if err := e.setBalance(order.GetBaseUnit(), order.GetUserId(), quantity, types.BalanceMinus); err != nil {
			return &response, err
		}

		e.trade(&order, types.AssigningBuy)
...
func (e *Service) GetOrders(ctx context.Context, req *pbspot.GetRequestOrders) (*pbspot.ResponseOrder, error) {
	...
	case types.AssigningBuy:
		maps = append(maps, fmt.Sprintf("where assigning = '%[1]v' and type = '%[2]v'", types.AssigningBuy, types.TypeSpot))
	case types.AssigningSell:
		maps = append(maps, fmt.Sprintf("where assigning = '%[1]v' and type = '%[2]v'", types.AssigningSell, types.TypeSpot))
	default:
		maps = append(maps, fmt.Sprintf("where (assigning = '%[1]v' and type = '%[3]v' or assigning = '%[2]v' and type = '%[3]v')", types.AssigningBuy, types.AssigningSell, types.TypeSpot))
	}
...
func (e *Service) GetAssets(ctx context.Context, _ *pbspot.GetRequestAssets) (*pbspot.ResponseAsset, error) {
...
func (e *Service) SetAsset(ctx context.Context, req *pbspot.SetRequestAsset) (*pbspot.ResponseAsset, error) {
	...
	case types.GroupCrypto:
		...
		row, err := e.Context.Db.Query(`select id from balances where symbol = $1 and user_id = $2 and type = $3`, req.GetSymbol(), auth, types.TypeSpot)
		if err != nil {
			return &response, err
		}
		defer row.Close()
...
func (e *Service) GetAsset(ctx context.Context, req *pbspot.GetRequestAsset) (*pbspot.ResponseAsset, error) {
	...
	_ = e.Context.Db.QueryRow(`select coalesce(sum(case when base_unit = $1 then value when quote_unit = $1 then value * price end), 0.00) as volume from orders where base_unit = $1 and status = $2 and type = $3 and user_id = $4 or quote_unit = $1 and status = $2 and type = $3 and user_id = $4`, req.GetSymbol(), types.StatusPending, types.TypeSpot, auth).Scan(&row.Volume)
	...
	for i := 0; i < len(row.GetChainsIds()); i++ {
		...
		if chain, _ := e.getChain(row.GetChainsIds()[i], false); chain.GetId() > 0 {
			...
			if contract, _ := e.getContract(row.GetSymbol(), row.GetChainsIds()[i]); contract.GetId() > 0 {
				...
				price, err := e.GetPrice(context.Background(), &pbspot.GetRequestPrice{BaseUnit: chain.GetParentSymbol(), QuoteUnit: req.GetSymbol()})
				if err != nil {
					return &response, err
				}
			...
			_ = e.Context.Db.QueryRow("select exists(select value as balance from balances where symbol = $1 and user_id = $2 and type = $3)::bool", req.GetSymbol(), auth, types.TypeSpot).Scan(&chain.Exist)
...
func (e *Service) GetPrice(_ context.Context, req *pbspot.GetRequestPrice) (*pbspot.ResponsePrice, error) {
...
// The function takes a context.Context and a *pbspot.GetRequestTransactions as parameters. The function returns a
...
func (e *Service) GetTransactions(ctx context.Context, req *pbspot.GetRequestTransactions) (*pbspot.ResponseTransaction, error) {
	...
	if response.GetCount() > 0 {
		...
		rows, err := e.Context.Db.Query(fmt.Sprintf(`select id, symbol, hash, value, price, fees, confirmation, "to", chain_id, user_id, assignment, "group", platform, protocol, status, error, create_at from transactions %s order by id desc limit %d offset %d`, strings.Join(maps, " "), req.GetLimit(), offset))
		if err != nil {
			return &response, err
		}
		defer rows.Close()
...
func (e *Service) SetWithdraw(ctx context.Context, req *pbspot.SetRequestWithdraw) (*pbspot.ResponseWithdraw, error) {
	...
	if contract.GetProtocol() != types.ProtocolMainnet {
		...
		price, err := e.GetPrice(context.Background(), &pbspot.GetRequestPrice{
	...
	if err := e.setBalance(req.GetSymbol(), auth, req.GetQuantity(), types.BalanceMinus); e.Context.Debug(err) {
		return &response, err
	}
...
func (e *Service) CancelWithdraw(ctx context.Context, req *pbspot.CancelRequestWithdraw) (*pbspot.ResponseWithdraw, error) {
	...
	if row.Next() {
		...
		if err := e.setBalance(item.GetSymbol(), item.GetUserId(), item.GetValue(), types.BalancePlus); e.Context.Debug(err) {
			return &response, err
		}
...
func (e *Service) CancelOrder(ctx context.Context, req *pbspot.CancelRequestOrder) (*pbspot.ResponseOrder, error) {
	...
	row, err := e.Context.Db.Query(`select id, value, quantity, price, assigning, base_unit, quote_unit, user_id, create_at from orders where status = $1 and type = $2 and id = $3 and user_id = $4 order by id`, types.StatusPending, types.TypeSpot, req.GetId(), auth)
	if err != nil {
		return &response, err
	}
	defer row.Close()
	...
	if row.Next() {
		...
		if _, err := e.Context.Db.Exec("update orders set status = $1 where type = $2 and id = $3 and user_id = $4;", types.StatusCancel, types.TypeSpot, item.GetId(), item.GetUserId()); err != nil {
			return &response, err
		}
		...
		case types.AssigningBuy:
			...
			if err := e.setBalance(item.GetQuoteUnit(), item.GetUserId(), decimal.New(item.GetValue()).Mul(item.GetPrice()).Float(), types.BalancePlus); err != nil {
				return &response, err
			}
		...
		case types.AssigningSell:
			...
			if err := e.setBalance(item.GetBaseUnit(), item.GetUserId(), item.GetValue(), types.BalancePlus); err != nil {
				return &response, err
			}
