...
func (m *Service) request(method string, content map[string]interface{}) (interface{}, error) {
	defer func() {
...
func (m *Service) message(data interface{}) {
			...
			if extract, ok := fields[3].([]interface{}); ok {
				...
				for i := 0; i < len(extract[0].([]interface{})); i++ {
					...
					if bid, ok := extract[0].([]interface{}); ok {
						...
						if len(columns) == 2 {
							response.Fields = append(response.Fields, &pbmarket.Depth_Attributes{
								Assigning: "BID",
